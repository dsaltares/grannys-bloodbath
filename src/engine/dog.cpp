...
Dog::Dog(Game *g, const char *path, int x, int y):Enemy(g){
...
void Dog::collides(Actor& a){
	
	Granny *granny_aux;
	
	try{
		granny_aux = static_cast<Granny*>(&a);
...
void Dog::normal_state(){
	
		state = MOVE;
}

void Dog::moving_state(){
...
void Dog::damaged_state(){
	
	if(animations[DAMAGED]->update())
		state = NORMAL;
}
void Dog::death_state(){
	
	if(animations[DIE]->update())
		state = ERASE;
}

void Dog::jumping_state(){
