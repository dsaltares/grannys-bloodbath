...
/**
	...
	Lleva el control de qué teclas están pulsadas en un momento determinado, cuáles se sueltan y cuáles se vuelven a presionar.
	...
	\code
		...
		while(!quit){
			...
			if(keyboard->released(Keyboard::KEY_HIT)) // Si dejamos de pulsar golpear
				...
			if(keyboard->newpressed(Keyboard::KEY_SHOOT)) // Si antes soltamos y ahora pulsamos disparar
		}
	\endcode
*/
class Keyboard{
	public:
		...
		/**
			@return Si es la primera vez que se usa Keyboard crea su instancia y la devuelve. Sino simplemente la devuelve.
		*/
		...
		/**
			Actualiza el estado del teclado. Debe llamarse una vez al comienzo del game loop.
		*/
		void update();
		...
		*/
		...
		/**
			...
			@return true si la tecla estaba antes pulsada en la última actualización y ahora no lo está, false en caso contrario.
		*/
		...
		/**
			...
			@return true si la tecla no estaba pulsada en la última actualización y ahora lo está, false en caso contrario.
		*/
		bool newpressed(keys key);
		
		/**
			@return true si se ha producido algún evento de salida, false en caso contrario
		*/
		bool quit();
		
		/**
		 *	Activa el flag para indicar que se desea salir de la aplicación
		 */
		void set_quit();
	...
	private:
		...
		bool _quit;
		std::map<keys, SDLKey> configured_keys;
		int n_keys;
};
