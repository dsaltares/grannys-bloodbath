...
Spider::Spider(Game *g):Enemy(g)
{
	energy = 0;
	force = 0;
}

Spider::Spider(Game *g, const char *path, int x, int y):Enemy(g)
{
	this->x = x;
	this->y = y;
	old_x = x;
	...
	bullet = new Bullet(g, "XML/spiderweb.xml", 0, 0, direction, Bullet::FAT);
}
...
void Spider::collides(Actor &a){}

Actor *Spider::clone(int x1, int y1)
{
	
	Spider *z = new Spider(g);
...
void Spider::normal_state()
{
	state = MOVE;
}

void Spider::moving_state(){
	...
	if(!within_range() && !in_view())
	{
		actual_vx = vx;
		if(direction){
			if(!g->on_ground(*this, x-vx-100, y)){
				change_direction();
			}	
		}
		else{
			
			if(!g->on_ground(*this, x + image->width()*2, y)){
				change_direction();
			}
		}
	}
...
void Spider::attackTOnormal_state()
{
...
void Spider::damaged_state()
{
	if(animations[DAMAGED]->update())
		state = NORMAL;
}

void Spider::death_state()
{
	if(animations[DIE]->update())
		state = ERASE;
}

void Spider::shoot_spiderweb()
{
	Bullet *b;
	attack->play();
	if(direction){
		b = static_cast<Bullet*>(bullet->clone(x , y + 10));
		b->set_direction(direction);
		g->add_bullet(b);
	}
	else{
		b = static_cast<Bullet*>(bullet->clone(x + image->width(), y + 10));
		b->set_direction(direction);
		g->add_bullet(b);
	}	
}

bool Spider::within_range()
{
	...
	if(positionx <= 350 && positiony <= 100){
